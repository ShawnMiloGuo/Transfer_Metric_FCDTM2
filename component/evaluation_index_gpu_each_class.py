...
def all_index(predictions, labels, num_classes, is_val=False):
    ...
    if is_val:
        ...
        precision_each_class = multiclass_precision(predictions, labels, num_classes, average="none").cpu().tolist()
        recall_each_class = multiclass_recall(predictions, labels, num_classes, average="none").cpu().tolist()
        F1_each_class = multiclass_f1_score(predictions, labels, num_classes, average="none").cpu().tolist()
        return OA, kappa, precision, recall, F1, miou, ious, precision_each_class, recall_each_class, F1_each_class

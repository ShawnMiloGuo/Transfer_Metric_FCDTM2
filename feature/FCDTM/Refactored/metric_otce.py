...
def compute_coupling(X_src, X_tar):
    ...
    cost_function = lambda x, y: geomloss.utils.squared_distances(x, y).cpu().numpy()
...
def test():
    ...
    for i in range(NUM_SAMPLE):
        src_x_list.append(np.random.randn(dimension))
        # tar_x_list.append(np.random.randn(512))

        src_y_list.append(np.random.randint(0,class_num))
        # tar_y_list.append(np.random.randint(0,10))
    #
    for i in range(NUM_SAMPLE_TAR):
        tar_x_list.append(np.random.randn(dimension))
        tar_y_list.append(np.random.randint(0,class_num))
    
    # the shape of x is n*512, and the shape of y is n*1 
    src_x = torch.tensor(np.array(src_x_list), dtype=torch.float) # torch.Size([100, 512])
    tar_x = torch.tensor(np.array(tar_x_list), dtype=torch.float)
    src_y = np.array(src_y_list)[:,np.newaxis] # (100, 1)
    ...
    print("W type:", type(W)) # <class 'numpy.float64'>
    print ('Wasserstein distance:%.4f:'%(W))

...
def extract_features_all_batch_low_memory(val_loader, model_device, model, label_index = 1):
    '''
    all_features
    '''
    all_feature_list_name = 'all_hook_output'
    ...
    for i in tqdm(range(len(val_loader))):
        ...
        if num_image > 300:
            break
    print(f"extracted_features: {len(extracted_features)}, {len(extracted_features[0])}")
...
def get_distribution_source_target(model_path, model_chdir,
                             data_path_source, data_path_target,
                             batch_size: int=1,
                             binary_class_index = 1,
                             label_1_percent = 0.2,
                             label_index_list = ["source", "target"],
                             ):
    # Load the model
    ...
    for label_index in label_index_list:
        iterator_val_loader = iter(val_loader_source) if label_index == "source" else iter(val_loader_target)
        extracted_features_dict[label_index] = extract_features_all_batch_low_memory(iterator_val_loader, model_device, model, label_index=1)
    ...
    with tqdm(total=col_subplots * row_subplots, desc="drawing") as pbar:                
        for col in range(col_subplots):
            for row in range(row_subplots):
                ...
                data_0 = extracted_features_dict[label_index_list[0]][:, outc_weight_desc_indices[dim_index]]
                #
                print_sum_num_0(data_0, dim_index)
                draw_normal_distribution(data_0, label=f"{label_index_list[0]}_label_1", plt=axs[col, row])

                data_1 = extracted_features_dict[label_index_list[1]][:, outc_weight_desc_indices[dim_index]]
                #
                print_sum_num_0(data_1, dim_index)
                draw_normal_distribution(data_1, label=f"{label_index_list[1]}_label_1", plt=axs[col, row])
...
def draw_distribution_one_dim(extracted_features_dict, 
    ...
    data_0 = np.mean(extracted_features_dict[label_index_list[0]], axis=mean_axis)
    ...
    data_1 = np.mean(extracted_features_dict[label_index_list[1]], axis=mean_axis)
    print(f"data_1.shape: {data_1.shape}")
    print_sum_num_0(data_1, print_sum_num_0_index)
    
    draw_normal_distribution(data_0, label=f"{label_index_list[0]}_label_1", plt=ax)
    draw_normal_distribution(data_1, label=f"{label_index_list[1]}_label_1", plt=ax)
...
def main(model_path_prefix,
    ...
    for dataset_i in dataset_i_list:
        ...
        # binary_class_index = 1
            ...
            label_index_list = ["source", "target"]
            extracted_features_dict, outc_weight, outc_weight_desc_indices = get_distribution_source_target(model_files[0], model_chdir,
                                        data_path_source, data_path_target,
            ...
            draw_distribution_all_dim(extracted_features_dict, 
                                    ...
                                    figname=f'{dataset_name_source}-to-{dataset_name_target}_cls_{binary_class_index}_distribution_all_dim.png',
            ...
            draw_distribution_one_dim(extracted_features_dict,
                                        result_path=os.path.join(result_path, "fig"),
                                        figname=f'{dataset_name_source}-to-{dataset_name_target}_cls_{binary_class_index}_distribution_one_dim-mean.png',
                                        label_index_list = label_index_list,
                                        mean_axis = 1,
                                        )
            draw_distribution_one_dim(extracted_features_dict,
                                        result_path=os.path.join(result_path, "fig"),
                                        figname=f'{dataset_name_source}-to-{dataset_name_target}_cls_{binary_class_index}_distribution_batch-mean.png',
                                        label_index_list = label_index_list,
                                        mean_axis = 0,
                                        )
...
if __name__ == "__main__":
    ...
    result_path = r"E:\Yiling\at_SIAT_research\z_result\20240526_draw_distribution_of_transfer\20240527_draw_distribution_of_transfer_1_dwqs2_xjs2_draw0"
    
    save_log(result_path=result_path, log_name="draw_distribution_of_transfer_dwqs2_xjs2.log")

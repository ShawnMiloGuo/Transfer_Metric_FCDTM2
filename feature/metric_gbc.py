...
def get_bhattacharyya_distance(per_class_stats, c1, c2, gaussian_type):
  ...
  mu1 = per_class_stats[c1]['mean'] # shape = (64), mu1, mu2, sigma1, sigma2
  ...
  if gaussian_type == 'spherical':
    sigma1 = tf.reduce_mean(sigma1) # shape = []
...
def compute_per_class_mean_and_variance(features, target_labels, unique_labels):
  ...
  for label in unique_labels:
    ...
    class_ids = tf.equal(target_labels, label) # cifa100, class_ids.shape = (50000,)
    class_features = tf.gather_nd(features, tf.where(class_ids)) # cifa100, class_features.shape = [500, 64]. features.shape = (50000, 64)
    mean = tf.reduce_mean(class_features, axis=0) # mean.shape = (64)
    ...
    for c2 in unique_labels:
      if c1 != c2:
        bhattacharyya_distance = get_bhattacharyya_distance(
            per_class_stats, int(c1), int(c2), gaussian_type) # bhattacharyya_distance.shape = []

...
def feature_and_index_batch(iterator_val_loader, model_device, model):
    ...
    return OA, F1, miou, precision, recall, feature_source_mean, feature_source_var, feature_source
...
def feature_and_index_all_batch(val_loader, model_device, model, source_or_target = 'source'):
    ...
    return OA_all, F1_all, miou_all, precision_all, recall_all, feature_mean, feature_var, all_features
...
def transfer_metric_all_all(model_path, model_chdir,
...
def transfer_metric_all_batch(model_path, model_chdir,
    ...
    # get mean difference, variance difference, ...
    # source domain
    OA_s, F1_s, miou_s, precision_s, recall_s, mean_s, var_s, all_features_s = feature_and_index_all_batch(iterator_val_loader_source, model_device, model)
    all_features_s_permute = all_features_s.permute(0, 2, 3, 1).reshape(-1, 64)
    # target domain
    target_domain_all = True
    if target_domain_all:
        OA_t, F1_t, miou_t, precision_t, recall_t, mean_t, var_t, all_features_t = feature_and_index_all_batch(iterator_val_loader_target, model_device, model)
    for i in tqdm(range(len(val_loader_target))):
        if not target_domain_all:
            OA_t, F1_t, miou_t, precision_t, recall_t, mean_t, var_t, all_features_t = feature_and_index_batch(iterator_val_loader_target, model_device, model)
        # mean, var, ...
        # mean_difference = (mean_s - mean_t).abs().mean().item()
        # F范数
        ...
        # Frechet Distance
        # all_features_s_permute = all_features_s.permute(0, 2, 3, 1).reshape(-1, 64)
        # sigma_s = torch.cov(all_features_s_permute.t())
        # all_features_t_permute = all_features_t.permute(0, 2, 3, 1).reshape(-1, 64)
        # sigma_t = torch.cov(all_features_t_permute.t())
        # fd = frechet_distance(mean_s, mean_t, sigma_s, sigma_t)
...
if __name__ == "__main__":
    ...
    result_path = r"E:\Yiling\at_SIAT_research\z_result\20240621_transfer_metric_FID\20240621_transfer_metric_FID_all-all_1_dwqs2-xjs2_batch4_300img"
    # result_path = r"E:\Yiling\at_SIAT_research\z_result\20240530_transfer_metric_FID\20240530_transfer_metric_FID_all-batch_1_dwqs2-xjs2_batch4"
    
    save_log(result_path=result_path, log_name="transfer_metric_dwqs2-xjs2.log")

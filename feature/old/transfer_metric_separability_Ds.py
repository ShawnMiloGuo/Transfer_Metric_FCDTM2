...
from transfer_metric_FD_mask0_all_all_label1_no_feature0 import get_y0_y1_diff
...
def get_last_layer_weight(model):
    ...
    model_last_layer_weight = model.outc.weight.detach().cpu().squeeze()
    last_layer_weight_dict = {i: model_last_layer_weight[i] for i in range(model_last_layer_weight.shape[0])}
    print(f"model_last_layer_weight.shape: {model_last_layer_weight.shape}")
    print(f"last_layer_weight_dict: {last_layer_weight_dict}")
    last_layer_weight_dict_abs_normalized = {i: torch.abs(model_last_layer_weight[i]) / torch.sum(torch.abs(model_last_layer_weight[i])) for i in range(model_last_layer_weight.shape[0])}
    print(f"last_layer_weight_dict_abs_normalized: {last_layer_weight_dict_abs_normalized}")
    return last_layer_weight_dict_abs_normalized
...
def main(model_path_prefix,
    ...
    for dataset_i in dataset_i_list:
        ...
        for binary_class_index in binary_class_index_list:
            ...
            # all-batch
            transfer_metric_all_batch_Ds(model_path=model_files[0], model_chdir=model_chdir,
...
def get_args():
    ...
    parser.add_argument('--result_path', type=str, help='Path to RESULT',
                        default=r'E:\Yiling\at_SIAT_research\z_result\20241119_transfer_metric_Ds_weighted\20241119_1726_debug')
    parser.add_argument('--log_name', type=str, default="transfer_metric_weighted_Ds_dwqs2-xjs2.log", help='log_name')
    ...
    parser.add_argument('--target_domain_all', type=int, default=0, help='1: all, 0: batch')

...
# FD feature extractor
def feature_and_index_all_batch(val_loader, model_device, model, label_index = None, no_feature0 = False, one_batch = False, by_pred = False):
    '''
    ...
    def one_batch_iter():
        ...
        if label_index is not None:
            if by_pred:
                batch_labels = predictions.cpu().unsqueeze(1).expand_as(feature_source).permute(0, 2, 3, 1)
            else:
                # use true_masks_cpu to extract the feature
    ...
    # no_feature0 = True
    if no_feature0:
        print("no_feature0 is True, calculate feature_mean and feature_var with non-zero features")
...
# DS feature extractor
# saparate the feature to foreground and background by prediciton (or true_masks)
def feature_list_from_pred_and_index_all_batch(val_loader, model_device, model, one_batch = False, by_pred = False):
    '''
    ...
    def one_batch_iter():
        ...
        # extract the feature of label_index
        batch_features = feature_source.permute(0, 2, 3, 1)
        # by prediciton (or true_masks)
        if by_pred:
            batch_pred = predictions.cpu().unsqueeze(1).expand_as(feature_source).permute(0, 2, 3, 1)
            batch_label = batch_pred
        else:
            batch_true_mask = true_masks_cpu.unsqueeze(1).expand_as(feature_source).permute(0, 2, 3, 1)
            batch_label = batch_true_mask
        for i in range(num_classes):
            extracted_batch_features = batch_features[batch_label == i].reshape(-1, feature_source.shape[1])
        ...
        # OA, F1, miou, precision, recall = all_index(predictions, true_masks, num_classes=2, ignore_label_0=True)
        index_list.append([OA, F1, miou, precision, recall])
    ...
    for i in range(num_classes):
        ...
        num_samples_dict[i] = extracted_features_tensor_dict[i].size(0)

    # feature_mean_all = torch.mean(torch.cat([extracted_features_tensor_dict[i] for i in range(num_classes)], dim=0), dim=0)
    total_samples = sum(num_samples_dict.values())
    feature_mean_all = sum(feature_mean_dict[i] * num_samples_dict[i] for i in range(num_classes)) / total_samples
    
    OA_all, F1_all, miou_all, precision_all, recall_all = torch.tensor(index_list).mean(dim=0).tolist()
    return OA_all, F1_all, miou_all, precision_all, recall_all, feature_mean_all, feature_mean_dict, extracted_features_dict

# GBC feature extractor
def feature_label_and_index_all_batch(val_loader, model_device, model, one_batch = False, by_pred = False):
    '''
    ...
    def one_batch_iter():
        ...
        with torch.no_grad():
            model_output = model(images)
        predictions = torch.argmax(model_output, dim=1) # [B, H, W]
        feature_source = feature_from_hook['hook_output'].cpu() # [B, C, H, W]
        # extract the feature and label
        batch_features = feature_source.permute(0, 2, 3, 1) # [B, H, W, C]
        if by_pred:
            batch_labels = predictions.cpu()
        else:
            batch_labels = true_masks_cpu # [B, H, W]
        extracted_features.extend(batch_features.reshape(-1, feature_source.shape[1]).tolist()) # [B*H*W, C]
        extracted_labels.extend(batch_labels.reshape(-1).tolist()) # [B*H*W]
...
def calculate_Dispersion_score_weighted_feature(mean_all, mean_dict, num_samples_dict, feature_weight_dict):
    num_classes = len(mean_dict)
    weighted_sum = 0.0
    for i in range(num_classes):
        weighted_sum += num_samples_dict[i] * torch.norm((mean_all - mean_dict[i]) * feature_weight_dict[i], p=2).item()
    score = weighted_sum / (num_classes - 1)
    log_score = np.log(score).item()
    return score, log_score
...
def transfer_metric_all_batch_mean_w_b(model_path, model_chdir,
    ...
    val_dataset_source = dataset_reader(root_dir=data_path_source, is_train=dataset_is_train, transform= None, binary_class_index = binary_class_index, label_1_percent = label_1_percent)
    val_loader_source = data.DataLoader(val_dataset_source, batch_size=batch_size, shuffle=False, num_workers=num_workers)

    val_dataset_target = dataset_reader(root_dir=data_path_target, is_train=dataset_is_train, transform= None, binary_class_index = binary_class_index, label_1_percent = label_1_percent)
    ...
    for i in tqdm(range(len(val_loader_target)), desc="features_one_batch"):
        ...
        mean_dif_key = ["mean_dif_absolute", 
                        "mean_dif_absolute_abs", 
                        "mean_dif_relative", 
                        "mean_dif_relative_abs",
                        ]
        mean_dif_dict = dict(zip(mean_dif_key, [mean_dif_absolute, 
                                                mean_dif_absolute_abs, 
                                                mean_dif_relative, 
                                                mean_dif_relative_abs,
                                                ]))
        ...
        for y_diff in y_diff_dict_key:
            fid_weighted = calculate_fid_mask_0(fid_mu1 * np.array(y_diff_dict[y_diff]), fid_sigma1, fid_mu2 * np.array(y_diff_dict[y_diff]), fid_sigma2)
            fid_dict[f"FD_{y_diff}"] = fid_weighted
        ...
        if i != 0:
            append_pre = ["", f"{i+1}", "", ""]
        OA_delta_relative = (OA_s - OA_t) / OA_s
        F1_delta_relative = (F1_s - F1_t) / F1_s
        precision_delta_relative = (precision_s - precision_t) / precision_s
        ...
        result_list.append(append_pre + 
                        [OA_s-OA_t, F1_s-F1_t, precision_s-precision_t,
                         ...
                         OA_s, F1_s, precision_s, OA_t, F1_t, precision_t,
                         ...
                         + [fid_dict[key] for key in fid_dict.keys()]
                         )
    ...
    result_list_name =["source", "target", "class_index", "class_name",  # 0-3
                        ...
                        "OA_delta_relative", "F1_delta_relative", "precision_delta_relative", # 7-9
                        "OA_s", "F1_s", "precision_s", "OA_t", "F1_t", "precision_t", # 10-15
                        "mean_dif_absolute_sum", "mean_dif_absolute_abs_sum", "mean_dif_relative_sum", "mean_dif_relative_abs_sum", # 16-19
                         ] + [f"{key}" for key in mean_dif_multify_y_diff_dict.keys() # 20-
                              ] + [f"{key}" for key in fid_dict.keys()] # 
...
def transfer_metric_all_batch_Ds(model_path, model_chdir,
    ...
    # get the weight of last layer
    # last_layer_weight_dict_abs_normalized = get_last_layer_weight(model)

    # get y0_y1_diff from "transfer_metric_FD_mask0_all_all_label1_no_feature0.py"
    y_diff_dict, y_diff_dict_key = get_y0_y1_diff(model)
    y0_y1_diff_abs = y_diff_dict["y0_y1_diff_abs"]
    weight_abs_normalized = y0_y1_diff_abs / torch.sum(y0_y1_diff_abs)
    weight_dict_abs_normalized = {i: weight_abs_normalized for i in range(num_classes)}
    print(f"weight_dict_abs_normalized: {weight_dict_abs_normalized}")

    # for i in range(model.outc.weight.shape[])
    ...
    OA_s, F1_s, miou_s, precision_s, recall_s, feature_mean_all_s, feature_mean_dict_s, all_extracted_features_dict_s = \
          feature_list_from_pred_and_index_all_batch(iterator_val_loader_source, model_device, model, one_batch=False)
    # all_features_s_permute = all_extracted_features_s # [N, feature_dim]
    # print(f"all_features_s_permute.shape: {all_features_s_permute.shape}")
    ...
    if target_domain_all:
        OA_t, F1_t, miou_t, precision_t, recall_t, feature_mean_all_t, feature_mean_dict_t, all_extracted_features_dict_t = \
              feature_list_from_pred_and_index_all_batch(iterator_val_loader_target, model_device, model, one_batch=False, by_pred=by_pred)
    for i in tqdm(range(len(val_loader_target)), desc="features_one_batch"):
        if not target_domain_all:
            OA_t, F1_t, miou_t, precision_t, recall_t, feature_mean_all_t, feature_mean_dict_t, all_extracted_features_dict_t = \
                  feature_list_from_pred_and_index_all_batch(iterator_val_loader_target, model_device, model, one_batch=True, by_pred=by_pred)
        ...
        # target_Dispersion_score_no_log_weighted, target_Dispersion_score_weighted = calculate_Dispersion_score_weighted_feature(feature_mean_all_t, feature_mean_dict_t, num_samples_dict_t, last_layer_weight_dict_abs_normalized)
        target_Dispersion_score_no_log_weighted, target_Dispersion_score_weighted = calculate_Dispersion_score_weighted_feature(feature_mean_all_t, feature_mean_dict_t, num_samples_dict_t, weight_dict_abs_normalized)
        ...
        result_list.append(append_pre + 
                        [OA_s, F1_s, miou_s, precision_s, recall_s, 
                            ...
                            target_Dispersion_score_no_log_weighted, target_Dispersion_score_weighted])
    ...
    result_list_name =["source", "target", "class_index", "class_name",  # 0-3
                        ...
                        "target_no_log_Dispersion_score", "target_Dispersion_score", # 19-20
                        "target_no_log_Dispersion_score_weighted", "target_Dispersion_score_weighted", # 21-22
                         ] 
...
# GBC
# transfer_metric_GBC
def transfer_metric_all_batch_GBC(model_path, model_chdir,
         data_path_source, data_path_target,
         batch_size: int=1,
         binary_class_index = 1, 
         label_1_percent = 0.2,
    ...
    OA_s, F1_s, miou_s, precision_s, recall_s, extracted_features_s, extracted_labels_s = \
          feature_label_and_index_all_batch(iterator_val_loader_source, model_device, model, one_batch=False, by_pred=by_pred)
    # all_features_s_permute = all_extracted_features_s # [N, feature_dim]
    # print(f"all_features_s_permute.shape: {all_features_s_permute.shape}")
    ...
    if target_domain_all:
        OA_t, F1_t, miou_t, precision_t, recall_t, extracted_features_t, extracted_labels_t = \
              feature_label_and_index_all_batch(iterator_val_loader_target, model_device, model, one_batch=False, by_pred=by_pred)
    for i in tqdm(range(len(val_loader_target)), desc="features_one_batch"):
        if not target_domain_all:
            OA_t, F1_t, miou_t, precision_t, recall_t, extracted_features_t, extracted_labels_t = \
                  feature_label_and_index_all_batch(iterator_val_loader_target, model_device, model, one_batch=True, by_pred=by_pred)
...
def main(model_path_prefix,
    ...
    for dataset_i in dataset_i_list:
        ...
        for binary_class_index in binary_class_index_list:
            ...
            # result of per class
            dict_key = f"{dataset_name_source}-{dataset_name_target}_cls_{binary_class_index}"
            ...
            if transfer_metric_name == "FD":
                ...
                # range_accuracy_i = range(4, 10)
                range_metric_i = [16, 17, 20, 22, 36] 
                range_accuracy_i = [4, 5] # "OA_delta", "F1_delta",
            elif transfer_metric_name == "DS":
                result_list_name = transfer_metric_all_batch_Ds(model_path=model_files[0], model_chdir=model_chdir,
                                        data_path_source=data_path_source, data_path_target=data_path_target,
                                        batch_size=batch_size, binary_class_index=binary_class_index, 
                                        label_1_percent=label_1_percent, 
                                        append_pre=[dataset_name_source, dataset_name_target, binary_class_index, binary_class_name_list[binary_class_index]],
                ...
                # range_accuracy_i = range(9, 19)
                range_metric_i = [20, 22]
                range_accuracy_i = [14, 15] # "OA_delta", "F1_delta",
            # elif transfer_metric_name == "OTCE":
            #     pass
            elif transfer_metric_name == "GBC":
                result_list_name = transfer_metric_all_batch_GBC(model_path=model_files[0], model_chdir=model_chdir,
                                        data_path_source=data_path_source, data_path_target=data_path_target,
                                        batch_size=batch_size, binary_class_index=binary_class_index, 
                                        label_1_percent=label_1_percent, 
                                        append_pre=[dataset_name_source, dataset_name_target, binary_class_index, binary_class_name_list[binary_class_index]],
                ...
                # range_accuracy_i = range(9, 19)
                range_metric_i = [20] # "spherical_GBC"
                range_accuracy_i = [14, 15] # "OA_delta", "F1_delta",
            else:
                print("main(): Unknown transfer metric name")
    ...
    with open(result_list_name_path, "w") as file:
        json.dump(result_list_name, file)
            ...
            for dataset_i in dataset_i_list:
                ...
                for binary_class_index in binary_class_index_list:
                    ...
                    draw_scatter_all_batch(result_list, x_col=metric_i, y_col=accuracy_i,
                                           ...
                                           figname=f"draw_{dataset_i}_{dataset_name_source_list[dataset_i]}-{dataset_name_target_list[dataset_i]}_cls-{binary_class_index}-{binary_class_name_list[binary_class_index]}_batch{batch_size}_{result_list_name[metric_i]}_{result_list_name[accuracy_i]}.png",
                                           save_fig=save_fig)
            plt.close()
...
def get_args():
    ...
    # by_pred
    parser.add_argument('--by_pred', type=int, default=0, help='by pred or true_label, 1: by pred, 0: by true_label')
    # by_pred
    parser.add_argument('--dataset_is_train', type=int, default=1, help='use train dataset or val dataset, 1: train, 0: val')

    # transfer_metric_name
    parser.add_argument('--transfer_metric_name', type=str, default="FD", help='transfer_metric_name, FD, DS, GBC')
...
if __name__ == "__main__":
    ...
    main(model_path_prefix = args.model_path_prefix,
            ...
            by_pred=args.by_pred, # by pred or true_label, True: by pred, False: by true_label
            transfer_metric_name=args.transfer_metric_name, # "FD", "DS"
            dataset_is_train=args.dataset_is_train, # use train dataset or val dataset, 1: train, 0: val
            )

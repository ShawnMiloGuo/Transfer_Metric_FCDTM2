...
def train_model(
    ...
    train_dateset = dataset_reader(root_dir=data_path, is_train=True, transform= None, binary_class_index=binary_class_index)
    train_loader = data.DataLoader(train_dateset, batch_size=batch_size, shuffle=True, num_workers=4)
    val_dataset = dataset_reader(root_dir=data_path, is_train=False, transform= None, binary_class_index=binary_class_index)
...
def get_args():
    ...
    # training parameters
    parser.add_argument('--n_epoch', type=int, default=200, help='Number of the epochs')
